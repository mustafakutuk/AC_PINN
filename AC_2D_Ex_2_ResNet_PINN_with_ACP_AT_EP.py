...
from deepxde.plotting import newfig, savefig
...
class PhysicsInformedNN:
    ...
    def check_models(self, val1, val2):
        a = np.zeros(len(val1))
        for i in range(0,len(val1)):
            if (val1[i]==val2[i]).all():
                a[i] = True
            else:
                a[i] = False

        if a.all()==True:
            print("Model parameters are same.")
        else:
            print("Model parameters are not same.")
...
if __name__ == "__main__":
    for i in range(0, 40):
        ...
        data = scipy.io.loadmat(
            'D:/Mustafa/Doctoral Thesis/PINNs-master/PINNs-master/main/Data/AC_2D_Time_Adaptivity_v2/AC_2D_40_timesteps.mat')
