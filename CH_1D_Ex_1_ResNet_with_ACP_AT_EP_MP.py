...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, X_mass, layers, lb, ub):
        ...
        # tf Graphs
        self.u0_pred, _, self.u0_xx_pred, self.m0_pred = self.net_u(self.x0_tf, self.t0_tf)
        self.u_lb_pred, self.u_x_lb_pred, _, self.m_lb_pred = self.net_u(self.x_lb_tf, self.t_lb_tf)
        self.u_ub_pred, self.u_x_ub_pred, _, self.m_ub_pred = self.net_u(self.x_ub_tf, self.t_ub_tf)
        _, self.m_x_lb_pred, _, _, _, _ = self.net_f_u(self.x_lb_tf, self.t_lb_tf)
        _, self.m_x_ub_pred, _, _, _, _ = self.net_f_u(self.x_ub_tf, self.t_ub_tf)
        self.u_pred, self.u_x_pred, self.u_xx_pred, self.m_pred = self.net_u(self.x_f_tf, self.t_f_tf)
        self.f_u_pred, _, _, _, _, _ = self.net_f_u(self.x_f_tf, self.t_f_tf)
        _, _, self.energy_pred, _, _, self.energy_t_pred = self.net_f_u(self.x_energy_tf, self.t_energy_tf)
        _, _, _, self.mass_pred, self.mass_t_pred, _ = self.net_f_u(self.x_m_tf, self.t_m_tf)


        self.loss = 100.0 * tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred) + tf.square(
                        self.m0_pred - (self.u0_tf ** 3 - self.u0_tf) + (0.02 * self.u0_xx_pred))) + \
                    tf.reduce_mean(tf.square(self.u_lb_pred - self.u_ub_pred) + tf.square(self.u_x_lb_pred - self.u_x_ub_pred)) + \
                    tf.reduce_mean(tf.square(self.m_lb_pred - self.m_ub_pred) + tf.square(self.m_x_lb_pred - self.m_x_ub_pred)) + \
                    tf.reduce_mean(tf.square(self.f_u_pred) + tf.square(self.m_pred - (self.u_pred ** 3 - self.u_pred) + (0.02 * self.u_xx_pred))) + \
                    tf.reduce_mean(tf.square(self.mass_t_pred)) + \
                    tf.reduce_mean(tf.square(tf.math.maximum(0.0, self.energy_t_pred)))
    ...
    def net_u(self, x, t):
        ...
        u_func = (u ** 3 - u) - (0.02 * u_xx)

        return u, u_x, u_xx, u_func

    def net_f_u(self, x, t):
        u, u_x, _, u_func = self.net_u(x, t)
        u_t = tf.gradients(u, t)[0]
        u_func_x = tf.gradients(u_func, x)[0]
        u_func_xx = tf.gradients(u_func_x, x)[0]

        f_u = u_t - u_func_xx

        energy = (1/self.x_energy.shape[0]) * (np.sum(0.01 * (np.abs(u_x) ** 2)) + (
            np.sum(0.25 * (1 - u ** 2) ** 2)))

        energy_t = tf.gradients(energy, t)[0]

        mass = (1 / 11.0) * np.sum(u)
        mass_t = tf.gradients(mass, t)[0]

        return f_u, u_func_x, energy, mass, mass_t, energy_t
    ...
    def train(self, nIter):
        ...
        for it in range(nIter):
            ...
            if it > 1000 and max(a[k - 1] - a[k - 2], a[k - 2] - a[k - 3], a[k - 3] - a[k - 4]) > tol:
                ...
                n = int(0.075 * self.x_f.shape[0])
                x_test = self.lb + (self.ub - self.lb) * lhs(2, n)
                u_pred_test, f_u_pred_test, _, _ = model.predict(x_test)
                ...
                u_lb_pred_test, f_u_lb_pred_test, _, _ = model.predict(X_lb_test)
                loss_lb_test = np.abs(f_u_lb_pred_test)

                total_err = sum(loss_test)
                threshold = total_err / 12
                tot_percent = 0
                index = 0
    ...
    def predict(self, X_star):
        ...
        tf_dict = {self.x_energy_tf: X_star[:, 0:1], self.t_energy_tf: X_star[:, 1:2]}
        f_energy = self.sess.run(self.energy_pred, tf_dict)

        tf_dict = {self.x_m_tf: X_star[:, 0:1], self.t_m_tf: X_star[:, 1:2]}
        f_mass = self.sess.run(self.mass_pred, tf_dict)

        return u_star, f_u_star, f_energy, f_mass
...
if __name__ == "__main__":
    ...
    for i in range(0, 10):
        ...
        x_m = np.linspace(-1.0, 1.0, 201)
        x_m = x_m.flatten()[:, None]
        t_m = np.linspace(0.1*i, 0.1*(i+1), 11)
        t_m = t_m.flatten()[:, None]

        X_m, T_m = np.meshgrid(x_m, t_m)
        X_mass = np.hstack((X_m.flatten()[:, None], T_m.flatten()[:, None]))
        ...
        if i == 0:
            u0 = np.cos(np.pi * x0) - np.exp(- 4 * ((np.pi * x0) ** 2))
            model = PhysicsInformedNN(x0, u0, tb, X_f, X_mass, layers, lb, ub)
            start_time = time.time()
            train_loss_adam, train_loss_lbfgs, elapsed_adam, elapsed_lbfgs, all_x, all_t, all_x_lb, all_t_lb, all_x_ub, all_t_ub = model.train(
                2500)
            elapsed = time.time() - start_time
            print('Training time: %.4f' % (elapsed))
            u_pred, f_u_pred, f_energy_t, f_mass = model.predict(X_star)
            ...
            scipy.io.savemat('mass_t0.mat', {"mass_t0": f_mass})
            scipy.io.savemat('x_mass_t0.mat', {"x_mass_t0": X_mass})
            ...
            u0, _, _, _ = np.array(globals()[name].predict(x0_star))
            globals()[name] = PhysicsInformedNN(x0, u0, tb, X_f, X_mass, layers, lb, ub)
            start_time = time.time()
            train_loss_adam, train_loss_lbfgs, elapsed_adam, elapsed_lbfgs, all_x, all_t, all_x_lb, all_t_lb, all_x_ub, all_t_ub = \
            globals()[name].train(2500)
            elapsed = time.time() - start_time
            print('Training time: %.4f' % (elapsed))
            u_predname = 'u_pred' + str(i)
            f_u_predname = 'f_u_pred' + str(i)
            energy_predname = 'energy' + str(i)
            mass_predname = 'mass' + str(i)
            globals()[u_predname], globals()[f_u_predname], globals()[energy_predname], globals()[mass_predname] = \
            globals()[name].predict(X_star)
            ...
            scipy.io.savemat('mass_t' + str(i) + '.mat', {"mass_t" + str(i): globals()[mass_predname]})
            scipy.io.savemat('x_mass_t' + str(i) + '.mat', {"x_mass_t" + str(i): X_mass})
            scipy.io.savemat('train_loss_adam' + str(i) + '.mat', {"train_loss_adam" + str(i): train_loss_adam})
            scipy.io.savemat('train_loss_lbfgs' + str(i) + '.mat', {"train_loss_lbfgs" + str(i): train_loss_lbfgs})
            scipy.io.savemat('elapsed_adam' + str(i) + '.mat', {"elapsed_adam" + str(i): elapsed_adam})
            scipy.io.savemat('elapsed_lbfgs' + str(i) + '.mat', {"elapsed_lbfgs" + str(i): elapsed_lbfgs})
