...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, y0, u0, X_f, layers, lb, ub):
        ...
        # tf Graphs
        self.u0_pred, _, _, _, _, _ = self.net_u(self.x0_tf, self.y0_tf, self.t0_tf)
        self.u_left, self.u_x_left_pred, self.u_y_left_pred, _, _, _ = self.net_u(self.x_left_tf, self.y_left_tf, self.t_left_tf)
        self.u_right, self.u_x_right_pred, self.u_y_right_pred, _, _, _ = self.net_u(self.x_right_tf, self.y_right_tf,
                                                                            self.t_right_tf)
        self.u_upper, self.u_x_upper_pred, self.u_y_upper_pred, _, _, _ = self.net_u(self.x_upper_tf, self.y_upper_tf,
                                                                            self.t_upper_tf)
        self.u_lower, self.u_x_lower_pred, self.u_y_lower_pred, _, _, _ = self.net_u(self.x_lower_tf, self.y_lower_tf,
                                                                            self.t_lower_tf)
        self.u_pred, self.u_x_pred, self.u_y_pred, self.u_xx_pred, self.u_yy_pred, self.m_pred = self.net_u(self.x_f_tf, self.y_f_tf, self.t_f_tf)
        ...
        self.loss = 100000 * (tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred))) + \
                    tf.reduce_mean(tf.square(self.u_lower - self.u_upper)) + \
                    tf.reduce_mean(tf.square(self.u_x_lower_pred - self.u_x_upper_pred)) + \
                    tf.reduce_mean(tf.square(self.u_y_lower_pred - self.u_y_upper_pred)) + \
                    tf.reduce_mean(tf.square(self.u_left - self.u_right)) + \
                    tf.reduce_mean(tf.square(self.u_x_left_pred - self.u_x_right_pred)) + \
                    tf.reduce_mean(tf.square(self.u_y_left_pred - self.u_y_right_pred)) + \
                    tf.reduce_mean(tf.square(self.f_u_pred) + tf.square(self.m_pred - (
                            0.0032 * (self.u_xx_pred + self.u_yy_pred) - 0.5 * (
                            tf.math.log(tf.clip_by_value(1 + self.u_pred, 1e-10, 2)) - tf.math.log(
                        tf.clip_by_value(1 - self.u_pred, 1e-10, 2))) + 1.9 * self.u_pred))) + \
    ...
    def init_update(self, x0, y0, u0, X_f, layers, lb, ub):
        ...
        self.u0_pred, _, _, _, _, _ = self.net_u(self.x0_tf, self.y0_tf, self.t0_tf)
        self.u_left, self.u_x_left_pred, self.u_y_left_pred, _, _, _ = self.net_u(self.x_left_tf, self.y_left_tf,
                                                                                  self.t_left_tf)
        self.u_right, self.u_x_right_pred, self.u_y_right_pred, _, _, _ = self.net_u(self.x_right_tf, self.y_right_tf,
                                                                                     self.t_right_tf)
        self.u_upper, self.u_x_upper_pred, self.u_y_upper_pred, _, _, _ = self.net_u(self.x_upper_tf, self.y_upper_tf,
        ...
        self.u_lower, self.u_x_lower_pred, self.u_y_lower_pred, _, _, _ = self.net_u(self.x_lower_tf, self.y_lower_tf,
                                                                                     self.t_lower_tf)
        self.u_pred, self.u_x_pred, self.u_y_pred, self.u_xx_pred, self.u_yy_pred, self.m_pred = self.net_u(self.x_f_tf,
                                                                                                            self.y_f_tf,
                                                                                                            self.t_f_tf)
        self.f_u_pred, self.energy_pred, _ = self.net_f_u(self.x_f_tf, self.y_f_tf, self.t_f_tf)
        _, _, self.energy_t_pred = self.net_f_u(self.x_energy_tf, self.y_energy_tf, self.t_energy_tf)
    ...
    def net_u(self, x, y, t):
        ...
        u_func = (0.0032 * (u_xx + u_yy) - 0.5 * (tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2)) - tf.math.log(tf.clip_by_value(1 - u, 1e-16, 2))) + 1.9 * u)

        return u, u_x, u_y, u_xx, u_yy, u_func

    def net_f_u(self, x, y, t):
        u, u_x, u_y, u_xx, u_yy, u_func = self.net_u(x, y, t)

        u_t = tf.gradients(u, t)[0]

        f_u = u_t - ((1 - u) * (1 + u) * u_func)


        energy = (1 / self.x_energy.shape[0]) * (np.sum(0.0008 * ((tf.math.abs(u_x) + tf.math.abs(u_y)) ** 2)) + np.sum(
            0.25 * ((1 + u) * tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2)) + (1 - u) * tf.math.log(
                tf.clip_by_value(1 - u, 1e-16, 2))) - 0.475 * (u ** 2)))

        energy_t = (1 / self.x_energy.shape[0]) * (-1) * (np.sum((1 - u ** 2) * tf.math.abs(
             0.0016 * ((u_xx + u_yy) - 0.25 * (tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2)) - tf.math.log(
                 tf.clip_by_value(1 - u, 1e-16, 2))) + 0.95 * u)) ** 2))
...
if __name__ == "__main__":
    for i in range(0, 10):
        ...
        # Domain bounds
        lb = np.array([0.0, 0.0, 1.0 * i])
        ub = np.array([2 * np.pi, 2 * np.pi, 1.0 * (i + 1)])
        ...
        init_data = scipy.io.loadmat(
            '/Data/ic_lambda_0_4.mat')
        u0 = init_data['u0'].flatten()[:, None]

        if i == 0:
            model = PhysicsInformedNN(x0, y0, u0, X_f, layers, lb, ub)
            scipy.io.savemat('u_t0.mat', {"u_t0": u0})
            scipy.io.savemat('x_t0.mat', {"x_t0": x0})
            scipy.io.savemat('y_t0.mat', {"y_t0": y0})
            start_time = time.time()
            train_loss_adam, train_loss_lbfgs, elapsed_adam, elapsed_lbfgs, x_all, y_all, t_all, x_left_all, y_left_all, t_left_all, x_right_all, y_right_all, t_right_all, x_upper_all, y_upper_all, t_upper_all, x_lower_all, y_lower_all, t_lower_all = model.train(3000)
