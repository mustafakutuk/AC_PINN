...
class PhysicsInformedNN:
    ...
    #Periodic BCs, wi=10^3
        self.loss = 1000 * tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred)) + \
                    tf.reduce_mean(tf.square(self.u_lower - self.u_upper) +
                                   tf.square(self.u_x_lower_pred - self.u_x_upper_pred) +
                                   tf.square(self.u_y_lower_pred - self.u_y_upper_pred)) + \
                    tf.reduce_mean(tf.square(self.u_left - self.u_right) +
                                   tf.square(self.u_x_left_pred - self.u_x_right_pred) +
                                   tf.square(self.u_y_left_pred - self.u_y_right_pred)) + \
    ...
    def net_f_u(self, x, y, t):
        ...
        f_u = u_t - 0.01 * (u_xx + u_yy) - u + 0.125 * (tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2-(1e-16))) - tf.math.log(tf.clip_by_value(1 - u, 1e-16, 2-(1e-16))))

        energy = (1 / self.x_energy.shape[0]) * (np.sum(0.005 * ((tf.math.abs(u_x) + tf.math.abs(u_y)) ** 2) + (
                    0.125 * ((1 + u) * tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2-(1e-16))) + (1 - u) * tf.math.log(
                tf.clip_by_value(1 - u, 1e-16, 2-(1e-16)))) - 0.5 * (u ** 2))))
...
if __name__ == "__main__":
    for i in range(0, 40):
        ...
        # Domain bounds
        lb = np.array([0.0, 0.0, 0.125 * i])
        ub = np.array([2 * np.pi, 2 * np.pi, 0.125 * (i + 1)])
        ...
        t = np.linspace(0.125 * i, 0.125 * (i + 1), 6)
        ...
        x0 = np.arange(0, 2 * np.pi + 0.01, step=(2 * np.pi / 200))
        y0 = np.arange(0, 2 * np.pi + 0.01, step=(2 * np.pi / 200))
        x0 = x0.flatten()[:, None]
        y0 = y0.flatten()[:, None]
        ...
        if i==0:
            x_circle = [np.pi/2, np.pi/4, np.pi/2, np.pi, (3 * np.pi)/2, np.pi, (3 * np.pi)/2]
            y_circle = [np.pi/2, (3 * np.pi)/4, (5 * np.pi)/4, np.pi/4, np.pi/4, np.pi, (3 * np.pi)/2]
            r_circle = [np.pi/5, (2 * np.pi)/15, (2 * np.pi)/15, np.pi/10, np.pi/10, np.pi/4, np.pi/4]
            f0 = np.zeros((len(x0), len(x_circle)))
            for h in range(len(x0)):
                for k in range(len(x_circle)):
                    if(np.sqrt((x0[h] - x_circle[k])**2 + (y0[h] - y_circle[k])**2) - r_circle[k]) < 0:
                        f0[h, k] = 2 * np.exp(-0.01/((np.sqrt((x0[h] - x_circle[k])**2 + (y0[h] - y_circle[k])**2) - r_circle[k])**2))
                    else:
                        f0[h, k] = 0

            u0 = (-1 + np.sum(f0, axis=1))
            u0 = u0.flatten()[:, None]
