...
class PhysicsInformedNN:
    ...
    def net_f_u(self, x, y, t):
        ...
        f_u = u_t - 100 * (x - 0.5) * u_y - 0.01 * (u_xx + u_yy) + 100 * (u ** 3 - u)

        energy = (1 / self.x_energy.shape[0]) * (np.sum(0.5 * (np.abs((x - 0.5) * u_y)) ** 2) + np.sum((5 * 10 ** (-5)) * ((np.abs(u_x) + np.abs(u_y)) ** 2)) + np.sum(0.25 * ((1 - (u ** 2)) ** 2)))
...
if __name__ == "__main__":
    for i in range(0, 12):
        noise = 0.0
        # Domain bounds
        lb = np.array([0.0, 0.0, 0.005 * i])
        ub = np.array([1.0, 1.0, 0.005 * (i + 1)])
        ...
        t = np.linspace(0.005 * i, 0.005 * (i + 1), 11)
        ...
        x0 = np.arange(0, 1.01, step=(1 / 100))
        y0 = np.arange(0, 1.01, step=(1 / 100))
        x0 = x0.flatten()[:, None]
        y0 = y0.flatten()[:, None]
        ...
        if i==0:
            u0 = (-1) * np.tanh((np.abs(x0 + y0 - 1) + np.abs(x0 - y0) - 0.1)/(np.sqrt(2) * 0.01))
            ...
            scipy.io.savemat('u0.mat', {"u0": u0})
            scipy.io.savemat('x0.mat', {"x0": x0})
            scipy.io.savemat('y0.mat', {"y0": y0})
