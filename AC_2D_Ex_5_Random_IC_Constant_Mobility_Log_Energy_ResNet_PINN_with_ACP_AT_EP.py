...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, y0, u0, X_f, layers, lb, ub):
        ...
        self.u_right, self.u_x_right_pred, self.u_y_right_pred = self.net_u(self.x_right_tf, self.y_right_tf,
                                                                            self.t_right_tf)
        self.u_upper, self.u_x_upper_pred, self.u_y_upper_pred = self.net_u(self.x_upper_tf, self.y_upper_tf,
                                                                            self.t_upper_tf)
        self.u_lower, self.u_x_lower_pred, self.u_y_lower_pred = self.net_u(self.x_lower_tf, self.y_lower_tf,
                                                                            self.t_lower_tf)
    ...
    def net_f_u(self, x, y, t):
        ...
        f_u = u_t - 0.0032 * (u_xx + u_yy) + 0.15 * (tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2)) - tf.math.log(tf.clip_by_value(1 - u, 1e-16, 2))) - 0.6*u

        energy = (1 / self.x_energy.shape[0]) * (np.sum(0.0008 * ((tf.math.abs(u_x) + tf.math.abs(u_y)) ** 2)) + np.sum(
                    0.075 * ((1 + u) * tf.math.log(tf.clip_by_value(1 + u, 1e-16, 2)) + (1 - u) * tf.math.log(tf.clip_by_value(1 - u, 1e-16, 2))) - 0.15 * (u ** 2)))
...
if __name__ == "__main__":
    for i in range(0, 20):
        ...
        init_data = scipy.io.loadmat('/Data/ic_lambda_1.mat')
